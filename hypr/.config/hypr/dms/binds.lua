...
-- Cycles between dwindle -> master -> scrolling -> dwindle ...
local layout_order = { "dwindle", "master", "scrolling" }
...
local function cycle_layout()
  ...
  for i, name in ipairs(layout_order) do
    if name == current then
      next_layout = layout_order[(i % #layout_order) + 1]
      break
    end
  end

  hl.config({ general = { layout = next_layout } })
  hl.notification.create({ text = "Layout: " .. next_layout, timeout = 1200, icon = "ok" })
end
